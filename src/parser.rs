...
impl Parser {
    ...
    fn root<
        'a,
        E: ParseError<&'a str> + FromExternalError<&'a str, anyhow::Error> + ContextError<&'a str>,
    >(
        i: &'a str,
    ) -> IResult<&'a str, ParserOp, E> {
        context(
            "container",
            alt((
                delimited(char('['), Parser::parse_container, char(']')),
    ...
    >(
        ...
        match res {
            Ok(res) => Ok(("", res)),
            Err(e) => Err(Error(FromExternalError::from_external_error(
                i,
                ErrorKind::TakeUntil,
                e,
            ))),
    ...
    fn parse_json<'a, A>(s: &'a str) -> Result<A>
    where
        A: Deserialize<'a>,
    {
